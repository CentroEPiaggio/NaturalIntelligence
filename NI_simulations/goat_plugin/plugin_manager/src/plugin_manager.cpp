...
void JointStateCallback(const sea_plugin::state_infoConstPtr &msg, int i)
{
  q[i] = msg->q;
  dq[i] = msg->dq;
  tau[i] = msg->tau;
...
void Initialization(ros::NodeHandle n_)
{
  ...
  for(int i = 0; i<joints_n; i++)
  {
    sub_joints_state[i] = n_.subscribe<sea_plugin::state_info>(topics_state[i], 10, boost::bind(JointStateCallback, _1, i));
